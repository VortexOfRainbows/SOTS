		...
		*/
        ...
        public override void PostAddRecipes()
        {
            ...
            for (int i = 0; i < Recipe.numRecipes; i++)
            {
				...
				Item sight;
				Item might;
                if (recipe.TryGetIngredient(ItemID.SoulofFright, out fright) 
					&& recipe.TryGetIngredient(ItemID.SoulofSight, out sight) 
					&& recipe.TryGetIngredient(ItemID.SoulofMight, out might) 
					&& recipe.createItem.type != ModContent.ItemType<FrostedKey>() 
					&& recipe.createItem.type != ItemID.TrueNightsEdge
					&& recipe.createItem.type < ItemID.Count)
                {
		...
		public static void TryDoingLiquidScreenEffects()
        {
            if (Main.netMode != NetmodeID.Server)
            {
				...
				int liquidEffectsToMake = (SOTS.Config.SanctuaryLagReduction || SOTS.Config.lowFidelityMode) ? 30 : 90;
                for (int i = 0; i < liquidEffectsToMake; ++i)
                {
                ...
                if (t.LiquidAmount > 0)
                {
                    if (t.LiquidType == LiquidID.Water)
                    {
						if(noTileAbove)
                        {
                            for (int a = 0; a < 3; a++)
                            {
                                Vector2 pos = new(i * 16, j * 16 - t.LiquidAmount / 255f * 16 + Main.rand.NextFloat(8, 28));
                                //Water steaming
                                ...
                                Vector2 velo = new(Main.rand.NextFloat(-0.5f, 0.5f), Main.rand.NextFloat(-0.2f, 0.05f));
