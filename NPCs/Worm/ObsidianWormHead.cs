...
namespace SOTS.NPCs.Worm
{
    public class ObsidianWormHead : ModNPC
    {	int despawn = 0;
		public override void SetStaticDefaults()
		{
			
			DisplayName.SetDefault("Obsidian Digger");
		}
        public override void SetDefaults()
        {
           
            npc.lifeMax = 320;        //this is the npc health
            npc.damage = 22;    //this is the npc damage
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("ObsidianWormBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("ObsidianWormTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
        ...
        {
            return Main.tile[(spawnInfo.spawnTileX), (spawnInfo.spawnTileY)].type == 56 ? 0.20f : 0f; //100f is the spown rate so If you want your NPC to be rarer just change that value less the 100f or something.
        }
		
		public override void NPCLoot()
		{
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, (ItemID.Obsidian), Main.rand.Next(4, 6));
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("ObsidianScale"), Main.rand.Next(4, 18));
			if((Main.rand.Next(9) == 0))
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, (ItemID.ObsidianSkull), 1);
		
			if((Main.rand.Next(19) == 0))
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, (ItemID.WaterWalkingBoots), 1);	
				
			if((Main.rand.Next(49) == 0))
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, (ItemID.LavaCharm), 1);	
				
			if((Main.rand.Next(19) == 0))
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height,  (ItemID.LavaBucket), 1);
		}	
