...
namespace SOTS.NPCs.TreasureSlimes
{
	public class MutagenTreasureSlime : TreasureSlime
	{
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
			...
			if (NPC.frame.Y > 0)
				drawPos.Y -= 2;
			spriteBatch.Draw(eye, drawPos, null, color, NPC.rotation, drawOrigin, NPC.scale, NPC.spriteDirection == -1 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0f);
            spriteBatch.Draw(pupil, drawPos + toPlayer.SNormalize() * 1.25f, null, color, NPC.rotation, drawOrigin2, NPC.scale * 0.8f, NPC.spriteDirection == -1 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0f);
        }
        public override void SetDefaults()
		{
			...
			NPC.npcSlots = 1f;
			Banner = NPC.type;
			BannerItem = ItemType<MutagenTreasureSlimeBanner>();
			LootAmt = 3;
		...
		{
			npcLoot.Add(ItemDropRule.Common(ItemID.MusketBall, 1, 10, 20));
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            base.HitEffect(hit);
			if(NPC.life <= 0)
				Gore.NewGore(NPC.GetSource_Death(), NPC.Center - new Vector2(7, 7), Vector2.UnitY * -2, ModGores.GoreType("Gores/MutagenTreasureGore"), NPC.scale);
        }
    }
}