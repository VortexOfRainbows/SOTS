using System;
using System.IO;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using Terraria;
using Terraria.ID;
using Terraria.ModLoader;


namespace SOTS.NPCs.Chess
{
	public class King : ModNPC
	{	int start = 0;
		int faze2 = 0;
		int faze3 = 0;
		int despawn = 0;
		public override void SetStaticDefaults()
		{
			
			DisplayName.SetDefault("King");
		}
		public override void SetDefaults()
		{
			npc.aiStyle = 0;
            npc.lifeMax = 27500;   //boss life
            npc.damage = 75;  //boss damage
            npc.defense = 4;    //boss defense
            npc.knockBackResist = 0f;
            npc.width = 58;
            npc.height = 78;
			 Main.npcFrameCount[npc.type] = 1;    //boss frame/animation
            npc.value = 100000;
            npc.npcSlots = 0f;
            npc.boss = true;
            npc.lavaImmune = true;
            npc.noGravity = true;
            npc.noTileCollide = false;
            npc.HitSound = SoundID.NPCHit3;
            npc.DeathSound = SoundID.NPCDeath7;
            npc.netAlways = false;
			bossBag = mod.ItemType("KingBag");
		}
        public override void ScaleExpertStats(int numPlayers, float bossLifeScale)
        {
            npc.lifeMax = (int)(npc.lifeMax * 0.66f);  //boss life scale in expertmode
            npc.damage = (int)(npc.damage * 1.25f);  //boss damage increase in expermode
        }
		public override void BossLoot(ref string name, ref int potionType)
		{ 
		potionType = ItemID.GreaterHealingPotion;
	
		if(Main.expertMode)
		{ 
		npc.DropBossBags();
		} 
		else 
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("KingTrinity"),Main.rand.Next(1, 4)); 
				}
				}
		public override void AI()
		{
		 Player player  = Main.player[npc.target];
		 start++;
		 
			 SOTSPlayer modPlayer = (SOTSPlayer)player.GetModPlayer(mod, "SOTSPlayer");	
			 
			
		 if(start >= 24920 || modPlayer.chessSkip == true)
		 {
			if(faze2 <= 12)
			{
			npc.ai[2]++;
			npc.dontTakeDamage = false;
			}
			
			npc.aiStyle = 0;
		}
		else
		{
			npc.aiStyle = 0;
			npc.dontTakeDamage = true;
			
		}
		{ //dashy attack #1
			if(npc.ai[2] >= 30)
			{
				if(player.position.X - 80 > npc.position.X && npc.ai[2] == 30)
				{
					npc.velocity.X += 40;
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, -5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, 5, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, -5, 326, 42, 1, 0);
				}
				if(player.position.X + 80 < npc.position.X && npc.ai[2] == 30)
				{
					npc.velocity.X -= 40;
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, -5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, 5, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, -5, 326, 42, 1, 0);
				}
				if(npc.ai[2] == 45)
				{
					npc.velocity.X = 0;
				}
				if(npc.ai[2] == 60)
				{
					
				if(player.position.Y -80 > npc.position.Y)
				{
					npc.velocity.Y += 23;
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, -5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, 5, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, -5, 326, 42, 1, 0);
				}
				if(player.position.Y +80 < npc.position.Y)
				{
					npc.velocity.Y -= 23;
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, -5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 5, 0, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, 5, 326, 42, 1, 0);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, -5, 326, 42, 1, 0);
				}
				
					npc.ai[2] = 0;
					faze2++;
				
				
				
				
				
				
				
				}
			}
		
				if(npc.ai[2] == 15)
				{
					npc.velocity.Y = 0;
				}
		}
				if(faze2 == 13)
				{
					npc.velocity.Y = 0;
				}
				if(faze2 >= 13 && faze3 <= 8)
				{
					faze2++;
					npc.aiStyle = 14;
				}
				if(faze2 == 60)
				{
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, 0, mod.ProjectileType("PumpkinSpawn"), 1, 1, 0);
					faze3++;
					faze2 = 14;
				}
				if(faze3 >= 8)
				{
					npc.rotation += 0.3f;
					npc.aiStyle = 0;
					faze3++;
					npc.dontTakeDamage = true;
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, Main.rand.Next(-9,10), Main.rand.Next(-9,10), mod.ProjectileType("PumpkinCrusher"), 33, 0, 0);
				if(faze3 >= 300)
				{
					faze2 = 0;
					faze3 = 0;
					npc.rotation = 0;
					npc.ai[2] = 0;
				}
				
		   
				
				
				}
				if(Main.player[npc.target].dead)
			{
			 despawn++;
			}
			if(despawn >= 720)
			{
			npc.active = false;
			}
		}
		
			
			
			
}
	}
