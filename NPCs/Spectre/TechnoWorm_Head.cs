...
{[AutoloadBossHead]
    public class TechnoWormHead : ModNPC
    {	int restrictor = 0;
		int freezeTime = 0;
		int despawn = 0;
        ...
        public override void SetDefaults()
        {
            ...
            npc.defense = 999;         //this is the npc defense
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("TechnoWormBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("TechnoWormTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
		 ...
		 public override void ScaleExpertStats(int numPlayers, float bossLifeScale)
        {
            ...
            npc.damage = (int)(npc.damage * 4f);  //boss damage increase in expermode
        }
