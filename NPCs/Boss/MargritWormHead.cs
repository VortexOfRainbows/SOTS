...
{
    public class MargritWormHead : ModNPC
    {	int ai1 = 0;
		int ai2 = 0;
		int despawn = 0;
        ...
        public override void SetDefaults()
        {
            ...
            npc.lifeMax = 2000;        //this is the npc health
            ...
            npc.height = 24; //this is where you put the npc sprite height.   important
            npc.boss = false;
            ...
            npc.npcSlots = 0;
            npc.netAlways = true;
			npc.boss = false; // b o s s  s u m m o n
			npc.aiStyle = 6;
        }
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("MargritWormBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("MargritWormTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
		...
		public override void NPCLoot()
		{
			/*
			int rand = -1;
			if(Main.rand.Next(3) != 2 || Main.expertMode)
			rand = Main.rand.Next(3);
			
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height,  (mod.ItemType("TheQueen")), 1);
			
			if(rand == 0)
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height,  (mod.ItemType("TheQueenHelmet")), 1);
		
			if(rand == 1)
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height,  (mod.ItemType("TheQueenChest")), 1);
		
			if(rand == 2)
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height,  (mod.ItemType("TheQueenLegs")), 1);
			*/
		}
        ...
        {
            Texture2D texture = ModContent.GetTexture("SOTS/Projectiles/MargritChain");  
                     
			for(int i = 0; i < 200; i++)
			{
				if(Main.npc[i].type == mod.NPCType("CrypticCarver2"))
				{
