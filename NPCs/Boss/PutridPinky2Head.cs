...
{[AutoloadBossHead]
    public class PutridPinky2Head : ModNPC
    {	int ai1 = 0;
		int ai2 = 0;
		int despawn = 0;
        ...
        public override void SetDefaults()
        {
            ...
            npc.lifeMax = 13750;        //this is the npc health
            npc.damage = 35;    //this is the npc damage
            npc.defense = 8;         //this is the npc defense
            npc.knockBackResist = 0f;
            npc.width = 38;
            npc.height = 46;
            npc.lavaImmune = false;       //this make the npc immune to lava
        ...
        public override bool PreAI()
        {
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0)
                {
                    ...
                    {
                       latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("PutridPinky2Body"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
					latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("PutridPinky2Tail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
		...
		public override void PostAI()
		{
			...
			{
			Projectile.NewProjectile(npc.Center.X, npc.Center.Y, Main.rand.Next(-10,9), Main.rand.Next(-10,9), mod.ProjectileType("PinkBullet"), 27, 0f, 0);
			}
			ai1++;
				if(ai1 >= 360)
				{
				...
				npc.velocity.Y = (float)((Math.Sin(rotation) * Speed) * -1);
