	...
	{	bool finishedRotating = false;
		...
		{	
			...
			Vector2 tripletVelocity2 = rotateVelocity.RotatedBy(MathHelper.ToRadians(-15));
				...
				{
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, rotateVelocity.X, rotateVelocity.Y, mod.ProjectileType("MargritBolt"), 22, 0, 0, 0f, 0f); //Spawning a projectile
					if(Main.expertMode && Main.rand.Next(2) == 0)
					{
					Projectile.NewProjectile(npc.Center.X, npc.Center.Y, tripletVelocity.X, tripletVelocity.Y, mod.ProjectileType("MargritBolt"), 15, 0, 0, 0f, 0f);	
					Projectile.NewProjectile(npc.Center.X, npc.Center.Y, tripletVelocity2.X, tripletVelocity2.Y, mod.ProjectileType("MargritBolt"), 15, 0, 0, 0f, 0f);	
					}
				}
				...
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, rotateVelocity.X * 0.5f, rotateVelocity.Y * 0.5f, mod.ProjectileType("MargritBolt"), 35, 0, 0, 0f, 0f); //Spawning a projectile
				}
				if(npc.ai[0] >= 380)
				{
				npc.ai[1] += 3.25f;
				npc.rotation += MathHelper.ToRadians(3.25f);
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, rotateVelocity.X * 0.9f, rotateVelocity.Y * 0.9f, mod.ProjectileType("MargritBolt"), 35, 0, 0, 0f, 0f); //Spawning a projectile
				}
				if(npc.ai[0] >= 400)
				{
					npc.ai[0] = 0;
