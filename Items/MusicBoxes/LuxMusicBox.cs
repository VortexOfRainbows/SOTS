...
namespace SOTS.Items.MusicBoxes
{
	public class LuxMusicBox : ModItem
	{
		...
		{
			DisplayName.SetDefault("Music Box (Lux)");
	...
	public class LuxMusicBoxTile : ModTile
	{
		...
		{
			Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 32, ModContent.ItemType<LuxMusicBox>());
		}
        ...
        public override void SpecialDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			if (frameX % 2 == 0 && frameY % 2 == 0) //top left tile
			{
				...
				for (int k = 0; k < total; k++)
				{
					...
					Color color = VoidPlayer.pastelAttempt(radians, false);
					color.A = 0;
