...
namespace SOTS.Items.Otherworld.Furniture
{
	public class HardlightTable : ModItem
	{
		...
		{
			Texture2D texture2 = mod.GetTexture("Items/Otherworld/Furniture/HardlightTableBase");
		...
		{
			Texture2D texture = mod.GetTexture("Items/Otherworld/Furniture/HardlightTableMiniOutline");
			Texture2D texture2 = mod.GetTexture("Items/Otherworld/Furniture/HardlightTableMiniFill");
	...
	public class HardlightTableTile : ModTile
	{
		...
		{
			if (Main.tile[i, j].frameX < 18 || Main.tile[i, j].frameX > 35 || Main.tile[i, j].frameY % 36 < 18) //make it only draw if correct frame to prevent extra iterations
				return;
			float uniquenessCounter = Main.GlobalTime * -100 + (i + j) * 5;
			Tile tile = Main.tile[i, j];
			Texture2D texture = mod.GetTexture("Items/Otherworld/Furniture/HardlightTableTileGlow");
		...
		public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
		{
			if (Main.tile[i, j].frameX < 18 || Main.tile[i, j].frameX > 35 || Main.tile[i, j].frameY % 36 < 18)
				return true;
			Texture2D texture = mod.GetTexture("Items/Otherworld/Furniture/HardlightTableOutline");
			Texture2D texture2 = mod.GetTexture("Items/Otherworld/Furniture/HardlightTableFill");
			Color color;
			color = WorldGen.paintColor((int)Main.tile[i, j].color()) * (100f / 255f);
			color.A = 0;
			Vector2 zero = new Vector2(Main.offScreenRange, Main.offScreenRange);
