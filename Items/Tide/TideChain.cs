...
{
	public class TideChain : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			Item.createTile = ModContent.TileType<TideChainTile>();
			Item.placeStyle = 0;
		}
	}	
	public class TideChainTile : ModTile
	{
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(127, 127, 127), Language.GetText("Mods.SOTS.Items.TideChain.DisplayName"));
		...
		public override void MouseOver(int i, int j)
		{
			...
			{
				player.cursorItemIconID = ModContent.ItemType<BWand>();
				player.noThrow = 2;
        ...
        public override bool RightClick(int i, int j)
        {
			...
			int key = ModContent.ItemType<BWand>();
        ...
        public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
        {
			if(Main.tile[i, j].TileFrameX >= 18 && !fail && !effectOnly)
				Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, (j + 6) * 16, 16, 16, ModContent.ItemType<BWand>(), 1);
		}
		...
		{
			if (!TextureAssets.Item[ModContent.ItemType<BWand>()].IsLoaded)
			{
				Main.instance.LoadItem(ModContent.ItemType<BWand>());
			}
			Texture2D texture = TextureAssets.Tile[Type].Value;
			Texture2D textureSword = TextureAssets.Item[ModContent.ItemType<BWand>()].Value;
			...
			for (int z = 2; z < maxLength; z ++)
			{
				...
				if (z == maxLength - 1)
				{
					...
					if(Main.tile[i,j].TileFrameX >= 18)
					{
						Main.spriteBatch.Draw(textureSword, previous + zero + speed.SafeNormalize(Vector2.Zero) * 20f - Main.screenPosition, null, Lighting.GetColor((int)pos.X / 16, (int)pos.Y / 16, Color.White), rotateTo.ToRotation() + MathHelper.ToRadians(45), textureSword.Size() / 2, 1.1f, SpriteEffects.None, 0f);
					}
				}
				else
				{
