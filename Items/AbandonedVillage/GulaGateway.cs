...
{
	public class GulaGateway : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			Item.createTile = ModContent.TileType<GulaGatewayTile>();
		}
	}	
	public class GulaGatewayTile : ModTile
	{
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(62, 22, 12), Language.GetText("Mods.SOTS.Tiles.GatewayTile.MapEntry"));
			//TileID.Sets.DisableSmartCursor[Type] = true;
			DustType = ModContent.DustType<Dusts.GulaDust>();
		}
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			float uniquenessCounter = Main.GlobalTimeWrappedHourly * -100 + (i + j) * 5;
			Tile tile = Main.tile[i, j];
			Texture2D texture = Mod.Assets.Request<Texture2D>("Items/AbandonedVillage/GulaGatewayTileGlow").Value;
			Rectangle frame = new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, 16);
			Color color;
			color = WorldGen.paintColor((int)Main.tile[i, j].TileColor) * (100f / 255f);
			color.A = 0;
			float alphaMult = 0.45f * (float)Math.Sin(MathHelper.ToRadians(uniquenessCounter));
