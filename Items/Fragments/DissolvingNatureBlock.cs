...
namespace SOTS.Items.Fragments
{
	public class DissolvingNatureBlock : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			Item.createTile = ModContent.TileType<DissolvingNatureTile>();
		}
		public override void AddRecipes()
		{
			CreateRecipe(20).AddIngredient<DissolvingNature>(1).Register();
		}
	}
	public class DissolvingNatureTile : ModTile
	{
		public override void SetStaticDefaults()
		{
			...
			//ItemDrop/* tModPorter Note: Removed. Tiles and walls will drop the item which places them automatically. Use RegisterItemDrop to alter the automatic drop if necessary. */ = ModContent.ItemType<DissolvingNatureBlock>();
        ...
        {
			Texture2D texture = mod.Assets.Request<Texture2D>("Assets/SpiritBlocks/NatureParticle").Value;
			Texture2D textureBlock = mod.Assets.Request<Texture2D>("Assets/SpiritBlocks/NatureBlockOutline").Value;
			Color color;
			...
			for (int k = 0; k < 16; k += 2)
			{
				...
				if ((int)Main.tile[i, j].Slope == 1)
					location.Y += k;

				if ((int)Main.tile[i, j].Slope == 2)
					location.Y += 15 - k;
