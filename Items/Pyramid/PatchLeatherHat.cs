	...
	{	
        ...
        {	
			player.setBonus = "Summons a flock of flying snakes to assist in combat";
				...
				if (Probe == -1)
				{
					Probe = Projectile.NewProjectile(player.position.X, player.position.Y, 0, 0, mod.ProjectileType("FlyingSnake"), 14, 0, player.whoAmI);
				}
				...
				if (Probe2 == -1)
				{
					Probe2 = Projectile.NewProjectile(player.position.X, player.position.Y, 0, 0, mod.ProjectileType("FlyingSnake"), 15, 0, player.whoAmI);
				}
				...
				if (Probe3 == -1)
				{
					Probe3 = Projectile.NewProjectile(player.position.X, player.position.Y, 0, 0, mod.ProjectileType("FlyingSnake"), 16, 0, player.whoAmI);
				}
