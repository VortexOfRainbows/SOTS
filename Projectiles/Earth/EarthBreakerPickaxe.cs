    ...
    {	
        ...
        public override void AI()
		{
            ...
            else
            {
                ...
                Dust dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 4, Projectile.position.Y + 4), Projectile.width - 8, Projectile.height - 8, SOTSUtils.TypeHelper.PixelDustType);
                dust.noGravity = true;
                dust.velocity *= 0.3f;
                dust.scale = 1.5f;
                dust.fadeIn = 6f;
        ...
        public void HitTiles()
        {
            ...
            for(int k = -2; k <= 2; k++)
            {
                for (int h = -2; h <= 2; h++)
                {
                    ...
                    else if (Main.myPlayer == Projectile.owner && (NPC.downedBoss2 || SOTSWorld.downedExcavator) && Framing.GetTileSafely(i, j).TileType != TileID.DemonAltar)
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 360; i += 20)
            {
                ...
                Dust dust = Dust.NewDustDirect(new Vector2(Projectile.Center.X - 4, Projectile.Center.Y - 4), 0, 0, SOTSUtils.TypeHelper.PixelDustType);
                dust.noGravity = true;
                dust.velocity *= 1.6f;
