        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			if(projectile.ai[0] < 15)
			{
				...
				for(int j = 0; j < 450; j++)
				{
					...
					for (int i = -1; i < 2; i += 2)
					{
						Vector2 circular = new Vector2(0, i * 0.33f * (15 - projectile.ai[0])).RotatedBy(projectile.velocity.ToRotation());
						Main.spriteBatch.Draw(texture2, projectile.Center + offset + circular - Main.screenPosition, new Rectangle(0, 0, 2, 4), new Color(255, 100, 100) * (projectile.ai[0] / 15f) * alphaMult, projectile.velocity.ToRotation(), drawOrigin2, 0.5f, SpriteEffects.None, 0f);
					}
				...
				for (int j = 0; j < 2; j++)
				{
					for (int i = 0; i < 4; i++)
					{
						...
						Main.spriteBatch.Draw(texture, projectile.Center + new Vector2(x, y) - Main.screenPosition, null, new Color(150, 100, 100, 0), projectile.rotation * (1 + j * 0.33f), drawOrigin, scale * (projectile.scale * 1.25f + projectile.ai[0] * 0.0125f) * (1 - j * 0.33f), SpriteEffects.None, 0f);
					}
				}
			}
			return false;
