...
{    
    public class CardiacCollapseArm : ModProjectile 
    {	
		...
		private int chargeTime = 210; //charge time in frames
		private int explosiveCountMin = 1; //amount of explosions minimum
		private int explosiveCountMax = 3; //amount of explosions max
					...
					if(rotationTimer <= 0) //collision
					{
						if(projectile.owner == Main.myPlayer)
						{
							...
							for(int i = 0; i < explosive; i++)
							{
								...
								Projectile.NewProjectile(positionX, positionY, 0, 0, mod.ProjectileType("HeartCrush"), projectile.damage, initialDamage, Main.myPlayer, 0f, 0f);
							}
						}
						projectile.Kill();
					}
