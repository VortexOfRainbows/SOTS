...
{    
    public class CrabClawArm : ModProjectile 
    {
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Crab Claw Arm");
		...
		private float maxDamage = 8; //total damage %, 1 for no increase, 5 for 500% at max charge, etc
		private int chargeTime = 240; //charge time in frames
		private int explosiveCountMin = 1; //amount of explosions minimum
		private int explosiveCountMax = 2; //amount of explosions max
		private float explosiveRange = 48; //distance between each explosion
		private float releaseTime = 300; //how long in frames until auto-release
					...
					if(rotationTimer <= 0) //collision
					{
						if(projectile.owner == Main.myPlayer)
						{
							...
							for(int i = 0; i < explosive; i++)
							{
								...
								Projectile.NewProjectile(positionX, positionY, 0, 0, mod.ProjectileType("BubbleCrush"), projectile.damage, initialDamage, Main.myPlayer, 0f, 0f);
							}
						}
						projectile.Kill();
					}
