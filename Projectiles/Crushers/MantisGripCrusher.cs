    ...
    public class MantisGripCrusher : CrusherProjectile 
    {
        ...
        public override bool UseCustomExplosionEffect(float x, float y, float dist, float rotation, float chargePercent, int index, int armNumber)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, velo, ModContent.ProjectileType<NatureBoltFriendly>(), Projectile.damage, Projectile.knockBack, Projectile.owner, spawnAt.X, spawnAt.Y);
			}
			return true;
		}
