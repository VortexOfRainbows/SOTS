	...
	public class EtherealFlame : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			for (int k = 0; k < trailPos.Length; k++)
			{
				...
				Color color = ColorHelper.Pastel(MathHelper.ToRadians(colorCounter * 2 + k * 6));
				color.A = 0;
			...
			Color color2 = ColorHelper.Pastel(MathHelper.ToRadians(colorCounter * 2));
			color2.A = 0;
			...
			if (Main.netMode != NetmodeID.Server)
			{
				if(!SOTS.Config.lowFidelityMode || Main.rand.NextBool(3))
				{
					if (!Main.rand.NextBool(3))
					{
						...
						rotational.X *= 0.5f;
						rotational.Y *= 1f;
						particleList.Add(new ColoredFireParticle(rotational * -1.0f, rotational * 0.8f - Projectile.velocity * 0.05f, Main.rand.NextFloat(-3f, 3f), Main.rand.NextFloat(-2f, 2f), Main.rand.NextFloat(1.1f, 1.2f), ColorHelper.Pastel(MathHelper.ToRadians(colorCounter * 2 + Main.rand.NextFloat(-1, 1)), true)));
					}
					if (Main.rand.NextBool(3))
                    {
						Vector2 rotational = new Vector2(0, -1.5f).RotatedBy(MathHelper.ToRadians(Main.rand.NextFloat(360f)));
						particleList.Add(new ColoredFireParticle(Vector2.Zero, rotational * 0.8f - Projectile.velocity * 0.2f, Main.rand.NextFloat(-3f, 3f), Main.rand.NextFloat(-2f, 2f), Main.rand.NextFloat(0.9f, 1.0f), ColorHelper.Pastel(MathHelper.ToRadians(colorCounter * 2 + Main.rand.NextFloat(-1, 1)), true)));
					}
				}
				colorCounter++;
				cataloguePos();
