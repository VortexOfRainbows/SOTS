...
{    
    public class DreamingSmog : ModProjectile
	{
        ...
        public override bool PreDraw(ref Color lightColor)
		{
			...
			{
				starWindUp2 = MathHelper.Clamp(1 - Projectile.timeLeft / 24f, 0, 1);
			}
			...
			if (starWindUp > 0)
			{
				...
				SOTSProjectile.DrawStar(Projectile.Center + offSet, color, 0.35f * colorMult, Projectile.rotation, MathHelper.ToRadians(Counter * 3 * Projectile.direction), 4, 10f * scaleMult, 6f * scaleMult, xCompress, 480, 2f * scaleMult, 1);
				SOTSProjectile.DrawStar(Projectile.Center + offSet * 2, color, 0.5f * colorMult, Projectile.rotation, MathHelper.ToRadians(-Counter * 2.4f * Projectile.direction), 4, 4f * scaleMult, 2, xCompress, 240, 0, 1);
				SOTSProjectile.DrawStar(Projectile.Center, color, 0.25f * colorMult, Projectile.rotation, MathHelper.ToRadians(-Counter * Projectile.direction), 2, 8f * scaleMult, 12, xCompress, 180, 0, 1);
				SOTSProjectile.DrawStar(Projectile.Center, color, 0.25f * colorMult, Projectile.rotation, MathHelper.ToRadians(-Counter * Projectile.direction) + MathHelper.PiOver2, 2, 8f * scaleMult, 16, xCompress, 210, 0, 1);
				for (int i = 0; i < 8; i++)
				{
					Vector2 circular = new Vector2(2f, 0).RotatedBy(MathHelper.ToRadians(45 * i));
					Main.spriteBatch.Draw(texture, Projectile.Center + circular - Main.screenPosition, null, color * 0.9f * starWindUp * starWindUp, 0, texture.Size() / 2, 0.5f + 0.5f * scaleMult, SpriteEffects.None, 0);
				}
			}
			DrawStars(colorMult);
			return false;
		...
		public void DrawStars(float alphaMult)
		{
			...
			for (int i = 0; i < npcVectors.Count; i++)
			{
				...
				DrawStarPointingTowardEnemy(npcVectors[i].center + new Vector2(0, -2), progress * alphaMult, 40 + add, npcVectors[i].Z, i);
				if (i > 3)
					add += 8;
			}
		}
		public void DrawStarPointingTowardEnemy(Vector2 npcCenter, float progress, float radius, float degrees, int i)
		{
			...
			if (starWindUp > 0)
			{
				...
				SOTSProjectile.DrawStar(drawPosition, color * 1f * starWindUp, starWindUp, toNPC.ToRotation(), 0f, 4, 1f * scaleMult, 0, 1f, 120, 0, 1);
				for (int j = 0; j < 4; j++)
				{
					Vector2 circular = new Vector2(2f, 0).RotatedBy(MathHelper.PiOver2 * j);
					Main.spriteBatch.Draw(texture, Projectile.Center + circular - Main.screenPosition, null, color * 0.5f * starWindUp * starWindUp, 0, texture.Size() / 2, 0.25f + 0.25f * scaleMult, SpriteEffects.None, 0);
				}
			}
		}
		public Vector2 getStarPosition(Vector2 npcCenter, float radius, float degrees)
		{
			...
			{
				float sProgress = 1 - Projectile.timeLeft / 30f;
				float size = 20 + 30 * (1 - Projectile.timeLeft / 30f);
			...
			circularPosition.Y *= 0.3f + 0.4f * Projectile.timeLeft / 90f;
		...
		public void DrawChainsBetween(Vector2 start, Vector2 destination, float progress, float degrees)
		{
			...
			if (Projectile.timeLeft < 30)
			{
				addedHeight += 1.8f * (1 - Projectile.timeLeft / 30f);
			}
			...
			if (progress > 0)
			{
				...
				Main.spriteBatch.Draw(textureGradient, start - Main.screenPosition, null, color * (0.45f * multiplier * (0.6f + 0.4f * addedHeight)), toOther.ToRotation(), new Vector2(1, 1), new Vector2(1f / textureGradient.Width * dist, addedHeight + 1f * (1 - multiplier)), SpriteEffects.None, 0);
			}
		}
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				float scaleMult = 1f;
				Vector2 FlowerVe = new Vector2(-90f, 0f).RotatedBy(MathHelper.ToRadians(i * 360f / total) + MathHelper.PiOver4);
				if (i % 2 != 0)
					scaleMult = 0.6f;
				...
				for (int i = 0; i < npcVectors.Count; i++)
				{
					...
					if (ID >= 0)
					{
						...
						if (isNPCValidTarget(npc))
						{
							...
							Vector2 position = Projectile.Center + new Vector2(90 + add, 0).RotatedBy(toOther.ToRotation());
        ...
        public override void AI()
		{
            ...
            {
				ManageNPCList();
				if(Projectile.timeLeft > 30 && Projectile.timeLeft < 78 && Counter % 3 == 0)
		...
		public void ManageNPCList()
		{
			...
			for (int i = 0; i < npcVectors.Count; i++)
			{
				...
				Vector2 position = getStarPosition(npcVectors[i].center, 40 + add, npcVectors[i].Z);
				if (npcVectors[i].Y >= 5)
				{
					...
					for (int j = 0; j < maxJ; j++)
					{
						...
						dust.velocity = dust.velocity * 0.2f * scaleFactor + outward;
						dust.noGravity = true;
						dust.fadeIn = 0.1f;
						dust.scale = dust.scale * 0.1f + scaleFactor;
					}
				}
				if (i > 3)
					add += 8;
