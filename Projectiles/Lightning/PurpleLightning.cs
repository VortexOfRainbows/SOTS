    ...
    {	
		...
		public override void AI()
		{
			...
			if(Main.myPlayer == projectile.owner)
			{
				...
				projectile.velocity.Y += (0.6f * (Main.rand.Next(-3, 4)));
				
				projectile.velocity.X += (0.6f * (Main.rand.Next(-3, 4)));
			}
		}
		public override void Kill(int timeLeft)
		{
			for(int i = 0; i < 35; i++)
			{
			int num1 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), 8, 8, 173);
			Main.dust[num1].noGravity = true;
			}
			...
			for(int i = startingChain; i < 200; i++)
			{
				if(Main.npc[i] != target && projectile.knockBack > 0 && !stopCheck)
				{
					...
					if(distance < 300f && !target2.friendly && target2.active)
					{
							...
							if(Main.myPlayer == projectile.owner)
							{
								Projectile.NewProjectile(shootFromX, shootFromY, shootToX * 0.5f, shootToY * 0.3f, projectile.type, projectile.damage, projectile.knockBack -= 0.4f, Main.myPlayer, 0f, 0f); //Spawning a projectile
							}
							stopCheck = true;
							break;
					}
			...
			for(int i = 0; i < startingChain; i++)
			{
				if(Main.npc[i] != target && projectile.knockBack < 1 && projectile.knockBack > 0 && !stopCheck)
				{
					...
					if(distance < 300f && !target2.friendly && target2.active)
					{
							...
							if(Main.myPlayer == projectile.owner)
							{
								Projectile.NewProjectile(shootFromX, shootFromY, shootToX * 0.3f, shootToY * 0.3f, projectile.type, projectile.damage, projectile.knockBack -= 0.4f, Main.myPlayer, 0f, 0f); //Spawning a projectile
							}
							stopCheck = true;
							break;
					}
