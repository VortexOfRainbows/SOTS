		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			if (Projectile.owner == Main.myPlayer)
			{
				...
				if (npcIndex != -1)
				{
					...
					if (!npc.friendly && npc.lifeMax > 5 && npc.active && !npc.dontTakeDamage)
					{
						Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, 0, 0, ModContent.ProjectileType<PurpleLightningZap>(), (int)(Projectile.damage * 0.7f) + 1, target.whoAmI, Projectile.owner, npc.whoAmI, Projectile.ai[1]);
					}
				}
			}
		}
