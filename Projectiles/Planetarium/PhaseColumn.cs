...
namespace SOTS.Projectiles.Planetarium
{
	public class PhaseColumn : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			if (runOnce)
				return false;
			Texture2D texture = Mod.Assets.Request<Texture2D>("Projectiles/Planetarium/PhaseColumn").Value;
				...
				if (Projectile.ai[0] > 0 && endHow == 0)
				{
					for (int i = 0; i < 3; i += 2)
					{
						if (Projectile.owner == Main.myPlayer)
						{
							...
							Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<PhaseColumn>(), Projectile.damage, 1f, Main.myPlayer, Projectile.ai[0] - 1);
						}
					}
				}
				Projectile.velocity *= 0f;
