...
{
	public class CataclysmLightningZap: ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			if (runOnce || !hit)
				return false;
			Texture2D texture = Mod.Assets.Request<Texture2D>("Projectiles/Planetarium/CataclysmTrail").Value;
			...
			for (int i = 0; i < dist; i++)
			{
				...
				if (npc.active && npc.Hitbox.Intersects(new Rectangle((int)addPos.X - 12, (int)addPos.Y - 12, 24, 24)) && !npc.friendly)
				{
					if (Projectile.owner == Main.myPlayer && Projectile.friendly)
						Projectile.NewProjectile(Projectile.GetSource_FromThis(), addPos.X, addPos.Y, Projectile.velocity.X, Projectile.velocity.Y, ModContent.ProjectileType<CataclysmBulletDamage>(), Projectile.damage, 2f, Main.myPlayer, (int)Projectile.knockBack, Projectile.ai[1] - 1);
