...
{    
    public class HardlightTrail : ModProjectile 
    {
		...
		public override bool PreDraw(ref Color lightColor)
		{
			if (runOnce)
				return true;
			Texture2D texture = Mod.Assets.Request<Texture2D>("Projectiles/Planetarium/HardlightTrail").Value;
			...
			for (int k = 1; k < trailPos.Length; k++)
			{
				...
				color = color.MultiplyRGBA(new Color(100, 120, 140, 0));
		...
		public override void AI()
		{
			if(Projectile.ai[1] != -1 && end == false)
			{
				...
				if(proj.active && proj.type == ModContent.ProjectileType<HardlightGlaive>() && proj.owner == Projectile.owner && (int)proj.ai[1] == Projectile.whoAmI)
				{
