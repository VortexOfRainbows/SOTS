    ...
    {	int worm = 0;
		...
		public override void AI()
		{
			...
			if(boom >= 1)
			{
				...
				{
					if(Main.projectile[i].type == mod.ProjectileType("Fusion1"))
					{
							 ...
							 for(int j = 0; j < 200; j++)
								{
								   ...
								   if(distance < 300f && !target.friendly && target.active)
								   {
									   if(projectile.ai[1] != 1) //Assuming you are already incrementing this in AI outside of for loop
									   {
										   ...
										   Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootToX, shootToY, mod.ProjectileType("Fusion2Bolt"), projectile.damage, 0, Main.myPlayer, 0f, 0f); //Spawning a projectile
			...
			{
			int num1 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 68);
