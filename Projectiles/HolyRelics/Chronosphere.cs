...
namespace SOTS.Projectiles.HolyRelics        //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
 
{
    public class Chronosphere : ModProjectile
    {	float shootX = 5;
		float shootY = 0;
		float shootX2 = 0;
		float shootY2 = -5;
		...
		{
			DisplayName.SetDefault("Chronosphere");
        ...
        public override void AI()
        {
            ...
            projectile.rotation += 11.11111111f;   
						...
						Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootX, shootY, mod.ProjectileType("TurnTimeBeam"), projectile.damage, 0, Main.myPlayer, 0f, 0f);
						Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, -shootX, -shootY, mod.ProjectileType("TurnTimeBeam"), projectile.damage, 0, Main.myPlayer, 0f, 0f);
						Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootX2, shootY2, mod.ProjectileType("TurnTimeBeam"), projectile.damage, 0, Main.myPlayer, 0f, 0f);
						Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, -shootX2, -shootY2, mod.ProjectileType("TurnTimeBeam"), projectile.damage, 0, Main.myPlayer, 0f, 0f);
