...
{    
    public class HeliosSlash : ModProjectile 
    {
        ...
        public override void AI()
        {
            ...
            {
				runOnce = false;
				SOTSUtils.PlaySound(SoundID.Item60, (int)Projectile.Center.X, (int)Projectile.Center.Y, 0.6f, 0.2f);
			}
            if (Projectile.ai[0] == -1)
            {
				...
				if(Projectile.timeLeft > 8)
				{
					for (int i = 0; i < 360; i += 40)
					{
						...
						dust.scale *= 1.5f;
						dust.fadeIn = 0.1f;
						dust.color = new Color(180, 100, 30, 0);
					}
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			for (int k = starting; k < Projectile.oldPos.Length; k++)
			{
				...
				Color toChange = new Color(140, 120, 40, 0);
				...
				Main.spriteBatch.Draw(texture, drawPos, null, color * 0.6f, Projectile.rotation, new Vector2(texture.Width / 2, texture.Width / 2), Projectile.scale * scale * 1.375f, SpriteEffects.None, 0f);
			}
			if(drawMain)
			{
				Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, Projectile.GetAlpha(new Color(255, 255, 255)), Projectile.rotation, drawOrigin, Projectile.scale * 1.375f, SpriteEffects.None, 0f);
			}
			return false;
		}
	}
