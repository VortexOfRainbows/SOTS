...
namespace SOTS.Projectiles.Celestial
{
	public class BabyLaser : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			float laserDist = 100;
			Vector2 center = Projectile.Center;
			Color color = new Color(100, 255, 100, 0) * 1.5f;
            float bonusAlphaMult = 1 - 1 * (counter / 28f);
			float alpha = ((255f - Projectile.alpha) / 255f);
            for (int i = 0; i < laserDist; i++)
            {
				center += Projectile.velocity.SafeNormalize(Vector2.Zero) * Projectile.width * 3f;
				Main.spriteBatch.Draw(texture, center - Main.screenPosition, null, color * alpha, Projectile.velocity.ToRotation(), origin, new Vector2(6f, 1.5f), SpriteEffects.None, 0f);
                Main.spriteBatch.Draw(texture, center - Main.screenPosition, null, Color.White * alpha * 0.5f * bonusAlphaMult, Projectile.velocity.ToRotation(), origin, new Vector2(3f, 1.25f), SpriteEffects.None, 0f);
                for (int j = 0; j < 2; j++)
				{
					float dir = j * 2 - 1;
					Vector2 offset = new Vector2(counter * 1f * dir, 0).RotatedBy(Projectile.velocity.ToRotation() + MathHelper.ToRadians(90));
					Main.spriteBatch.Draw(texture, center - Main.screenPosition + offset, null, color * bonusAlphaMult * alpha, Projectile.velocity.ToRotation(), origin, new Vector2(3f, 1.5f), SpriteEffects.None, 0.0f);
				}
			}
			return false;
		}
