		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			if (Projectile.owner == Main.myPlayer)
			{
				...
				if (npcIndex != -1)
				{
					...
					if (!npc.friendly && npc.lifeMax > 5 && npc.active && !npc.dontTakeDamage)
					{
						Projectile.NewProjectile(Projectile.Center.X, Projectile.Center.Y, 0, 0, Mod.Find<ModProjectile>("ArcLightningZap").Type, (int)(Projectile.damage * 0.8f) + 1, target.whoAmI, Projectile.owner, npc.whoAmI, Projectile.ai[1]);
					}
				}
			}
		}
