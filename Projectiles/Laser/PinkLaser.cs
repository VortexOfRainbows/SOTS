	...
	public class PinkLaser : ModProjectile
	{
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) 
		{
			...
			for (float Distance = 0; Distance <= length; Distance += 6f) 
			{
				...
				if (!WorldGen.InWorld(i, j, 20) || Main.tile[i, j].active() && Main.tileSolidTop[Main.tile[i, j].type] == false && Main.tileSolid[Main.tile[i, j].type] == true && Main.tile[i, j].nactive())
				{
					break;
				}
